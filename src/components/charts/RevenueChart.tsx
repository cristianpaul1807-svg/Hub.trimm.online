...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-lg text-xs">
      <p className="font-black text-slate-400 mb-3">{label}</p>
...
export default function RevenueChart({ data, branches = [], loading }: RevenueChartProps) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-200">
      <p className="text-sm font-black text-slate-900 mb-6 uppercase tracking-widest">Revenue by Period</p>
      <ResponsiveContainer width="100%" height={280}>
        <AreaChart data={data} margin={{ top: 8, right: 0, left: -20, bottom: 0 }}>
          ...
          {branches.length > 0 && <Legend wrapperStyle={{ fontSize: 11, color: '#64748b', fontWeight: 600 }} />}
          <Area type="monotone" dataKey="total" stroke="#2563eb" strokeWidth={2.5} fill="url(#colorTotal)" name="Total" />
          {branches.map((b, i) => (
            <Area 
              key={b} 
              type="monotone" 
              dataKey={b} 
              stroke={BRANCH_COLORS[i + 1] || '#3b82f6'} 
              strokeWidth={1.5} 
              fill="none" 
              name={b} 
              strokeDasharray="4 2"
